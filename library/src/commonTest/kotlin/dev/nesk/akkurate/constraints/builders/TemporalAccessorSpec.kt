...
fun Duration.toDatePeriod(): DatePeriod {
    val period = this.toDateTimePeriod()

    return DatePeriod(period.years, period.months, period.days)
}


operator fun LocalDateTime.plus(duration: Duration) =
    (this.toInstant(TimeZone.UTC) + duration).toLocalDateTime(TimeZone.UTC)

operator fun LocalDateTime.minus(duration: Duration) =
    (this.toInstant(TimeZone.UTC) - duration).toLocalDateTime(TimeZone.UTC)

operator fun LocalDate.plus(duration: Duration) =
    this + duration.toDatePeriod()

operator fun LocalDate.minus(duration: Duration) =
    this - duration.toDatePeriod()

class FixedClock(fixedInstant: Instant, tz: TimeZone) : Clock {
    private val fixedInstant: Instant = fixedInstant.toLocalDateTime(tz).toInstant(tz)
...
class TemporalAccessorSpec : FunSpec() {
    companion object {
        private val fewSeconds = 5.seconds
        private val fewDays = 2.days
    }

    private val presentInstant: Instant = Clock.System.now()
    private val fixedClock: Clock = FixedClock(presentInstant, TimeZone.UTC)
    private val presentLocalDate = fixedClock.now().toLocalDateTime(TimeZone.UTC).date
    private val presentLocalDateTime = fixedClock.now().toLocalDateTime(TimeZone.UTC)
    ...
    init {
        ...
        fun `'Instant isInPast' fails when the value is in the future or present`() {
            assertFalse(
                Validatable(presentInstant + fewSeconds).isInPast().satisfied,
                "The constraint is not satisfied when the value is in the future"
            )
            assertFalse(
                Validatable(presentInstant).isInPast().satisfied,
                "The constraint is not satisfied when the value is present"
            )
        }
        ...
        fun `'LocalDate isInPast' fails when the value is in the future or present`() {
            assertFalse(
                Validatable(presentLocalDate + fewDays).isInPast().satisfied,
                "The constraint is not satisfied when the value is in the future"
            )
            assertFalse(
                Validatable(presentLocalDate).isInPast().satisfied,
                "The constraint is not satisfied when the value is present"
            )
        }
        ...
        fun `'LocalDateTime isInPast' fails when the value is in the future or present`() {
            assertFalse(
                Validatable(presentLocalDateTime + fewSeconds).isInPast().satisfied,
                "The constraint is not satisfied when the value is in the future"
            )
            assertFalse(
                Validatable(presentLocalDateTime).isInPast().satisfied,
                "The constraint is not satisfied when the value is present"
            )
        }
...
//            assertFalse(
//                Validatable(presentZonedDateTime + fewSeconds).isInPast().satisfied,
//                "The constraint is not satisfied when the value is in the future"
//            )
//            assertFalse(
//                Validatable(presentZonedDateTime).isInPast().satisfied,
//                "The constraint is not satisfied when the value is present"
//            )
//        }
        //endregion
        ...
        fun `'Instant isInPastOrIsPresent' succeeds when the value is in the past or present`() {
            assertTrue(
                Validatable(presentInstant - fewSeconds).isInPastOrIsPresent().satisfied,
                "The constraint is satisfied when the value is in the past"
            )
            assertTrue(
                Validatable(presentInstant).isInPastOrIsPresent().satisfied,
                "The constraint is satisfied when the value is present"
            )
        }
        ...
        fun `'LocalDate isInPastOrIsPresent' succeeds when the value is in the past or present`() {
            assertTrue(
                Validatable(presentLocalDate - fewDays).isInPastOrIsPresent().satisfied,
                "The constraint is satisfied when the value is in the past"
            )
            assertTrue(
                Validatable(presentLocalDate).isInPastOrIsPresent().satisfied,
                "The constraint is satisfied when the value is present"
            )
        }
        ...
        fun `'LocalDateTime isInPastOrIsPresent' succeeds when the value is in the past or present`() {
            assertTrue(
                Validatable(presentLocalDateTime - fewSeconds).isInPastOrIsPresent().satisfied,
                "The constraint is satisfied when the value is in the past"
            )
            assertTrue(
                Validatable(presentLocalDateTime).isInPastOrIsPresent().satisfied,
                "The constraint is satisfied when the value is present"
            )
        }
...
//            assertTrue(
//                Validatable(presentZonedDateTime - fewSeconds).isInPastOrIsPresent().satisfied,
//                "The constraint is satisfied when the value is in the past"
//            )
//            assertTrue(
//                Validatable(presentZonedDateTime).isInPastOrIsPresent().satisfied,
//                "The constraint is satisfied when the value is present"
//            )
//        }
//
...
//        }
        ...
        fun `'Instant isInFuture' fails when the value is in the past or present`() {
            assertFalse(
                Validatable(presentInstant - fewSeconds).isInFuture().satisfied,
                "The constraint is not satisfied when the value is in the past"
            )
            assertFalse(
                Validatable(presentInstant).isInFuture().satisfied,
                "The constraint is not satisfied when the value is present"
            )
        }
        ...
        fun `'LocalDate isInFuture' fails when the value is in the past or present`() {
            assertFalse(
                Validatable(presentLocalDate - fewDays).isInFuture().satisfied,
                "The constraint is not satisfied when the value is in the past"
            )
            assertFalse(
                Validatable(presentLocalDate).isInFuture().satisfied,
                "The constraint is not satisfied when the value is present"
            )
        }
        ...
        fun `'LocalDateTime isInFuture' fails when the value is in the past or present`() {
            assertFalse(
                Validatable(presentLocalDateTime - fewSeconds).isInFuture().satisfied,
                "The constraint is not satisfied when the value is in the past"
            )
            assertFalse(
                Validatable(presentLocalDateTime).isInFuture().satisfied,
                "The constraint is not satisfied when the value is present"
            )
        }
...
//            assertFalse(
//                Validatable(presentZonedDateTime - fewSeconds).isInFuture().satisfied,
//                "The constraint is not satisfied when the value is in the past"
//            )
//            assertFalse(
//                Validatable(presentZonedDateTime).isInFuture().satisfied,
//                "The constraint is not satisfied when the value is present"
//            )
//        }
        //endregion
        ...
        fun `'Instant isInFutureOrIsPresent' succeeds when the value is in the future or present`() {
            assertTrue(
                Validatable(presentInstant + fewSeconds).isInFutureOrIsPresent().satisfied,
                "The constraint is satisfied when the value is in the past"
            )
            assertTrue(
                Validatable(presentInstant).isInFutureOrIsPresent().satisfied,
                "The constraint is satisfied when the value is present"
            )
        }
        ...
        fun `'LocalDate isInFutureOrIsPresent' succeeds when the value is in the future or present`() {
            assertTrue(
                Validatable(presentLocalDate + fewDays).isInFutureOrIsPresent().satisfied,
                "The constraint is satisfied when the value is in the past"
            )
            assertTrue(
                Validatable(presentLocalDate).isInFutureOrIsPresent().satisfied,
                "The constraint is satisfied when the value is present"
            )
        }
        ...
        fun `'LocalDateTime isInFutureOrIsPresent' succeeds when the value is in the future or present`() {
            assertTrue(
                Validatable(presentLocalDateTime + fewSeconds).isInFutureOrIsPresent().satisfied,
                "The constraint is satisfied when the value is in the past"
            )
            assertTrue(
                Validatable(presentLocalDateTime).isInFutureOrIsPresent().satisfied,
                "The constraint is satisfied when the value is present"
            )
        }
...
//            assertTrue(
//                Validatable(presentZonedDateTime + fewSeconds).isInFutureOrIsPresent().satisfied,
//                "The constraint is satisfied when the value is in the past"
//            )
//            assertTrue(
//                Validatable(presentZonedDateTime).isInFutureOrIsPresent().satisfied,
//                "The constraint is satisfied when the value is present"
//            )
//        }
//
...
//        }
        ...
        fun `'Instant isBefore' fails when the value is after or equal to the provided one`() {
            assertFalse(
                Validatable(presentInstant).isBefore(presentInstant - fewSeconds).satisfied,
                "The constraint is not satisfied when the value is after the provided one"
            )
            assertFalse(
                Validatable(presentInstant).isBefore(presentInstant).satisfied,
        ...
        fun `'LocalDate isBefore' fails when the value is after or equal to the provided one`() {
            ...
            assertFalse(
                Validatable(presentLocalDate).isBefore(presentLocalDate).satisfied,
                "The constraint is not satisfied when the value is equal to the provided one"
            )
        }
        ...
        fun `'LocalDateTime isBefore' fails when the value is after or equal to the provided one`() {
            ...
            assertFalse(
                Validatable(presentLocalDateTime).isBefore(presentLocalDateTime).satisfied,
                "The constraint is not satisfied when the value is equal to the provided one"
            )
        }
...
//            )
//            assertFalse(
//                Validatable(presentZonedDateTime).isBefore(presentZonedDateTime).satisfied,
//                "The constraint is not satisfied when the value is equal to the provided one"
//            )
//        }
        //endregion
        ...
        fun `'Instant isBeforeOrEqualTo' succeeds when the value is before or equal to the provided one`() {
            assertTrue(
                Validatable(presentInstant).isBeforeOrEqualTo(presentInstant + fewSeconds).satisfied,
                "The constraint is satisfied when the value is before the provided one"
            )
            assertTrue(
                Validatable(presentInstant).isBeforeOrEqualTo(presentInstant).satisfied,
        ...
        fun `'LocalDate isBeforeOrEqualTo' succeeds when the value is before or equal to the provided one`() {
            ...
            assertTrue(
                Validatable(presentLocalDate).isBeforeOrEqualTo(presentLocalDate).satisfied,
                "The constraint is satisfied when the value is equal to the provided one"
            )
        }
        ...
        fun `'LocalDateTime isBeforeOrEqualTo' succeeds when the value is before or equal to the provided one`() {
            ...
            assertTrue(
                Validatable(presentLocalDateTime).isBeforeOrEqualTo(presentLocalDateTime).satisfied,
                "The constraint is satisfied when the value is equal to the provided one"
            )
        }
...
//            )
//            assertTrue(
//                Validatable(presentZonedDateTime).isBeforeOrEqualTo(presentZonedDateTime).satisfied,
//                "The constraint is satisfied when the value is equal to the provided one"
//            )
//        }
//
...
//        }
        ...
        fun `'Instant isAfter' fails when the value is before or equal to the provided one`() {
            assertFalse(
                Validatable(presentInstant).isAfter(presentInstant + fewSeconds).satisfied,
                "The constraint is not satisfied when the value is before the provided one"
            )
            assertFalse(
                Validatable(presentInstant).isAfter(presentInstant).satisfied,
                "The constraint is not satisfied when the value is equal to the provided one"
            )
        }
        ...
        fun `'LocalDate isAfter' fails when the value is before or equal to the provided one`() {
            ...
            assertFalse(
                Validatable(presentLocalDate).isAfter(presentLocalDate).satisfied,
                "The constraint is not satisfied when the value is equal to the provided one"
            )
        }
        ...
        fun `'LocalDateTime isAfter' fails when the value is before or equal to the provided one`() {
            ...
            assertFalse(
                Validatable(presentLocalDateTime).isAfter(presentLocalDateTime).satisfied,
                "The constraint is not satisfied when the value is equal to the provided one"
            )
        }
...
//            )
//            assertFalse(
//                Validatable(presentZonedDateTime).isAfter(presentZonedDateTime).satisfied,
//                "The constraint is not satisfied when the value is equal to the provided one"
//            )
//        }
        //endregion
        ...
        fun `'Instant isAfterOrEqualTo' succeeds when the value is after or equal to the provided one`() {
            assertTrue(
                Validatable(presentInstant).isAfterOrEqualTo(presentInstant - fewSeconds).satisfied,
                "The constraint is satisfied when the value is after the provided one"
            )
            assertTrue(
                Validatable(presentInstant).isAfterOrEqualTo(presentInstant).satisfied,
                "The constraint is satisfied when the value is equal to the provided one"
            )
        }
        ...
        fun `'LocalDate isAfterOrEqualTo' succeeds when the value is after or equal to the provided one`() {
            ...
            assertTrue(
                Validatable(presentLocalDate).isAfterOrEqualTo(presentLocalDate).satisfied,
                "The constraint is satisfied when the value is equal to the provided one"
            )
        }
        ...
        fun `'LocalDateTime isAfterOrEqualTo' succeeds when the value is after or equal to the provided one`() {
            ...
            assertTrue(
                Validatable(presentLocalDateTime).isAfterOrEqualTo(presentLocalDateTime).satisfied,
                "The constraint is satisfied when the value is equal to the provided one"
            )
        }
...
//            )
//            assertTrue(
//                Validatable(presentZonedDateTime).isAfterOrEqualTo(presentZonedDateTime).satisfied,
//                "The constraint is satisfied when the value is equal to the provided one"
//            )
//        }
//

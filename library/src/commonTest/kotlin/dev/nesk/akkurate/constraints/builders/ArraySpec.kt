...
class ArraySpec : FunSpec() {
    ...
    init {
        ...
        fun `'hasSizeEqualTo' fails when the size is different than the provided one`() {
            assertFalse(
                VALIDATABLE.hasSizeEqualTo(SIZE_MINUS_ONE).satisfied,
                "The constraint is not satisfied when the size is greater than the provided one"
            )
            assertFalse(
                VALIDATABLE.hasSizeEqualTo(SIZE_PLUS_ONE).satisfied,
                "The constraint is not satisfied when the size is lower than the provided one"
            )
        }
        ...
        fun `'hasSizeNotEqualTo' succeeds when the size is different than the provided one`() {
            assertTrue(
                VALIDATABLE.hasSizeNotEqualTo(SIZE_MINUS_ONE).satisfied,
                "The constraint is satisfied when the size is greater than the provided one"
            )
            assertTrue(
                VALIDATABLE.hasSizeNotEqualTo(SIZE_PLUS_ONE).satisfied,
                "The constraint is satisfied when the size is lower than the provided one"
            )
        }
        ...
        fun `'hasSizeLowerThan' fails when the size is greater than or equal to the provided one`() {
            assertFalse(
                VALIDATABLE.hasSizeLowerThan(SIZE_MINUS_ONE).satisfied,
                "The constraint is not satisfied when the size is greater than the provided one"
            )
            assertFalse(
                VALIDATABLE.hasSizeLowerThan(EXACT_SIZE).satisfied,
                "The constraint is not satisfied when the size is equal to the provided one"
            )
        }
        ...
        fun `'hasSizeLowerThanOrEqualTo' succeeds when the size is lower than or equal to the provided one`() {
            assertTrue(
                VALIDATABLE.hasSizeLowerThanOrEqualTo(SIZE_PLUS_ONE).satisfied,
                "The constraint is satisfied when the size is lower than the provided one"
            )
            assertTrue(
                VALIDATABLE.hasSizeLowerThanOrEqualTo(EXACT_SIZE).satisfied,
                "The constraint is satisfied when the size is equal to the provided one"
            )
        }
        ...
        fun `'hasSizeGreaterThan' fails when the size is lower than or equal to the provided one`() {
            assertFalse(
                VALIDATABLE.hasSizeGreaterThan(SIZE_PLUS_ONE).satisfied,
                "The constraint is not satisfied when the size is lower than the provided one"
            )
            assertFalse(
                VALIDATABLE.hasSizeGreaterThan(EXACT_SIZE).satisfied,
                "The constraint is not satisfied when the size is equal to the provided one"
            )
        }
        ...
        fun `'hasSizeGreaterThanOrEqualTo' succeeds when the size is greater than or equal to the provided one`() {
            assertTrue(
                VALIDATABLE.hasSizeGreaterThanOrEqualTo(SIZE_MINUS_ONE).satisfied,
                "The constraint is satisfied when the size is greater than the provided one"
            )
            assertTrue(
                VALIDATABLE.hasSizeGreaterThanOrEqualTo(EXACT_SIZE).satisfied,
                "The constraint is satisfied when the size is equal to the provided one"
            )
        }

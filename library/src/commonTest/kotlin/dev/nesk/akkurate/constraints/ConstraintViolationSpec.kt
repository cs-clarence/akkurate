...
class ConstraintViolationSpec : FunSpec() {
    init {

        test("'component1' returns the message") {
            assertEquals("foo", ConstraintViolation("foo", listOf("bar")).component1())
        }


        test("'component2' returns the path") {
        ...
        test("'equals' returns false when at least one of the values differ (variant 'path')") {
            val original = ConstraintViolation("foo", listOf("bar"))
        ...
        test("'hashCode' returns different hashes when at least one of the values differ (variant 'path')") {
            val original = ConstraintViolation("foo", listOf("bar"))
            val other = ConstraintViolation("foo", listOf(""))

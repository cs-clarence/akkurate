...
class ConstraintViolationSetSpec : FunSpec() {
    init {

        test("the contents are the same than the set provided to the constructor") {
            // Arrange
        ...
        test("the grouped messages contain the same data and in the same order") {
            // Arrange
            val originalSet = setOf(
                ConstraintViolation("message1", listOf("root", "child1")),
                ConstraintViolation("message2", listOf("root", "child2")),
            ...
            assertContentEquals(
                expectedMap.values,
                errors.values,
                "In each group, the messages keep the same order as the original collection"
            )
        }
        ...
        test("'equals' returns true when the messages are the same") {
        ...
        test("'equals' returns false when the messages differ") {
            val original = ConstraintViolationSet(setOf(ConstraintViolation("foo", listOf("bar"))))
        ...
        test("'hashCode' returns the same hash when the messages are the same") {
            val original = ConstraintViolationSet(setOf(ConstraintViolation("foo", listOf("bar"))))
            val other = ConstraintViolationSet(setOf(ConstraintViolation("foo", listOf("bar"))))
        ...
        test("'hashCode' returns different hashes when the messages differ") {
            val original = ConstraintViolationSet(setOf(ConstraintViolation("foo", listOf("bar"))))
            val other = ConstraintViolationSet(emptySet())

...
class ValidatableSpec : FunSpec() {
    init {

        test("the path of a root validatable is empty") {
            assertEquals(emptyList(), Validatable("foo").path())
        }


        test("the path segment gets appended to the parent path") {
        ...
        test("the validatable can be unwrapped with its original value") {
            val foo = object {}
            assertSame(foo, Validatable(foo).unwrap())
        }


        test("the first destructuring component unwraps with the original value") {
            val foo = object {}
            assertSame(foo, Validatable(foo).component1())
        }


        test("calling 'register' with a constraint stores it in the registry") {
            // Arrange
        ...
        test("calling 'register' with a constraint violation stores it in the registry") {
            // Arrange
            val registry = ConstraintRegistry(Configuration())
            val validatable = Validatable(null, registry)
        ...
        test("invoking the validatable executes the block immediately") {
            // Arrange
        ...
        test("calling 'validatableOf' returns a property wrapped in a Validatable") {
            // Arrange
            ...
            assertEquals(
                ...
                child.path(),
                "The child validatable extends the parent path with the property name"
            )
        }


        test("calling 'validatableOf' with a nullable receiver returns a nullable property wrapped in a Validatable (variant with non-null value)") {
            // Arrange
        ...
        test("calling 'validatableOf' with a nullable receiver returns a nullable property wrapped in a Validatable (variant with null value)") {
            // Arrange
        ...
        test("calling 'validatableOf' returns a getter function wrapped in a Validatable") {
            // Arrange
            val parent = Validatable("foo", "string")
            // Act
        ...
        test("calling 'validatableOf' with a nullable receiver returns a nullable getter function wrapped in a Validatable (variant with non-null value)") {
            // Arrange
            val parent = Validatable("foo" as String?, "string")
            // Act
            val child = parent.validatableOf(String::count)
        ...
        test("calling 'validatableOf' with a nullable receiver returns a nullable getter function wrapped in a Validatable (variant with null value)") {
            // Arrange
            val parent = Validatable(null as String?, "string")
            // Act
            val child = parent.validatableOf(String::count)
        ...
        test("'equals' returns true when the wrapped values are the same") {
        ...
        test("'equals' returns false when the wrapped values are different") {
        ...
        test("'hashCode' returns the same hash when the wrapped values are the same") {
            val validatable1 = Validatable("foo", "bar")
            val validatable2 = Validatable("foo", "baz", validatable1)
        ...
        test("'hashCode' returns different hashes when the wrapped values differ") {
            val validatable1 = Validatable("foo")
            val validatable2 = Validatable("bar")
